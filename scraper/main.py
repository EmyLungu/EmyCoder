...
SCRAPER_DIR = Path(__file__).resolve(strict=True).parent

load_dotenv()
TOKEN = os.getenv("GITHUB_TOKEN")

LANGUAGES = ["py", "c", "cpp", "rs", "js", "java"]
...
class Scraper:
    ...
    def add_row(self, content: str, extenstion: str, repo: str) -> None:
        self.data.append(
            {"Content": content, "Language": extenstion, "Repo": repo}
        )
    ...
    def process_file(self, repo: str, content) -> None:
        ...
        if extenstion.lower() not in LANGUAGES:
            return
    ...
    def update_log(self) -> None:
        ...
        if logfile.exists():
            with logfile.open(mode="r") as f:
                scrape_count_line = f.readline().strip()
                scrape_count = int(scrape_count_line.split("=")[-1])
        else:
            scrape_count = 0

        with logfile.open(mode="w+") as f:
            f.write(f"SCRAPE_COUNT={scrape_count + 1}")
